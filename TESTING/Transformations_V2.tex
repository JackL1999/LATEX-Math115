...
Also pay attention to which transformations come first before others. \\
...
Shrink by $1/2$ & Stretch by $5$ & Stretch by $2$ & - - - - - - - - - - - \\
\hrule & \hrule & \hrule & \hrule \\
Horizontal Shift & Horizontal Shift & Horizontal Shift & Horizontal Shift \\
Right by $1$ & - - - - - - - - - - - & Left by $1$ & Right by $3$\\
\hrule & \hrule & \hrule & \hrule \\
Vertical Shift & Vertical Shift & Vertical Shift & Vertical Shift \\
Upward by $2$ & Upward by $4$ & Downward by $2$ & Upward by $4$\\
\hrule & \hrule & \hrule & \hrule \\
Reflecton & Reflecton & Reflecton & Reflecton \\
- - - - - - - - - - - & - - - - - - - - - - - & - - - - - - - - - - - & Over the x-axis \\
\hrule & \hrule & \hrule & \hrule \\
\hline
\end{tabular}
...
\vspace{4cm}
\section*{Transformation given a point}


\begin{tabular}{|p{17.3cm}|}
\hline
A Transformation in the form $y=af(x-h)+k$ with a point $(x_0,y_0)$ from the original \\
function $f(x)$ will have the following point as the result $(x-h, ay+k)$ \\
Notice that only the x-value gets shifted and the y value remains the same then transformed \\
\hline
\end{tabular}
\\\\\\
If $(3,4)$ is a point on the graph $f(x)$, using the same transformations as before, what ordered pair 
must also be on the graph of $y$? We do not know the original function, but what can we do?
\\\\
\begin{tabular}{|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{4cm}|}
\hline
$y = \displaystyle\frac{1}{2}f(x-1)+2$ & $y=5f(x)+3$ & $y=-2f(x+1)-2$ & $y=f(x-3)+4$ \\
\hrule & \hrule & \hrule & \hrule \\
Let's use $(3,4)$ & Let's use $(3,4)$ & Let's use $(3,4)$ & Let's use $(3,4)$\\
\hrule & \hrule & \hrule & \hrule\\
x-value change: & x-value change: & x-value change: & x-value change: \\
$x_0+h=x_1$ & $x_0+h=x_1$ & $x_0-1=x_1$ & $x_0+3=x_1$ \\
$3+1=4$ & $3+0=3$ & $3-1=2$ & $3+3=6$ \\
\hrule & \hrule & \hrule & \hrule \\
y-value change: & y-value change: & y-value change: & y-value change: \\
$ay+k=y_1$ & $ay+k=y_1$ & $ay+k=y_1$ & $ay+k=y_1$ \\
$\frac{1}{2}(4)+2=4$ & $5(4)+3=23$ & $-2(4)-2=-10$ & $4+4=8$ \\
\hrule & \hrule & \hrule & \hrule \\
Final point & Final point & Final point & Final point \\
$(x_0,y_0)\rightarrow(x_1,y_1)$ & $(x_0,y_0)\rightarrow(x_1,y_1)$ & $(x_0,y_0)\rightarrow(x_1,y_1)$ & $(x_0,y_0)\rightarrow(x_1,y_1)$ \\
$(3,4)\rightarrow(4,4)$ & $(3,4)\rightarrow(3,23)$ & $(3,4)\rightarrow(2,-10)$ & $(3,4)\rightarrow(6,8)$ \\
\hrule & \hrule & \hrule & \hrule  \\
\hline
\end{tabular}

%% ADD REFLECTIONS TO THE TABLE
%% TWEAK VALUE FOR HORIZONTAL SHIFT
\noindent
...
\begin{minipage}[c]{0.40\textwidth} % Minipage for all graphs
    ...
    \draw[gray!15,step=1cm] (-5,-5) grid (5,5);
        ...
        \draw[red,line width=2pt,latex-latex] plot[domain= -2.2:2.2] (\x,{2*(\x*\x-2)});

    \end{tikzpicture}

    \vspace{4cm} % Add some space between graphs
    ...
    \draw[gray!15,step=1cm] (-5,-5) grid (5,5);
        ...
        \draw[red,line width=2pt,latex-latex] plot[domain= -5:5] (\x,{abs(\x) - 2});
    \end{tikzpicture}
...
\begin{minipage}[c]{0.40\textwidth} % Minipage for all graphs
    ...
    \draw[gray!15,step=1cm] (-5,-5) grid (5,5);
        ...
        \draw[red,line width=2pt,latex-latex] plot[domain= -0.5:4.5] (\x,{-1*(((\x-2)^2) - 1)});
    ...
    % === GRAPH 5 ===
    Adjust $y=k(x)$ below with transformation $y=-k(x)-2$
    \begin{tikzpicture}
	[
		scale=0.55,
		vector style/.style={->, very thick}
    ...
    \draw[gray!15,step=1cm] (-5,-5) grid (5,5);
        ...
        \draw[line width=0.5mm,  -latex] (0,-5.2) -- (0,5.2) node[above] {$y$};
        \foreach \y in {-5,-4,-3,-2,-1,1,2,3,4,5} \draw (.1,\y)--(-.1,\y) node[left] {\footnotesize $\y$};
        \draw[cyan,line width=2pt,latex-latex] plot[domain= -5:5, samples=100] (\x,{-1*abs(\x) + 2});
        \draw[red,line width=2pt,latex-latex] plot[domain= -5:5] (\x,{abs(\x) - 5});
    \end{tikzpicture}
